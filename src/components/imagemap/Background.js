...
class Background extends Component {
	...
	shouldComponentUpdate(nextProps, nextState) {
		if (JSON.stringify(this.state.avatars) !== JSON.stringify(nextState.avatars)) {
			return true;
		} else if (JSON.stringify(this.state.filteredAvatars) !== JSON.stringify(nextState.filteredAvatars)) {
			return true;
	...
	handlers = {
		...
		onSearchNode: e => {
			const filteredAvatars = this.handlers
				.transformList()
		...
		transformList: () => {
			return Object.values(this.props.avatars).reduce((prev, curr) => prev.concat(curr), []);
		},
		onSVGModalVisible: () => {
	...
	renderItem = (item, centered) =>{
		if(item.type === 'drawing'){
			...
			<div
				key={item.name}
				onClick={e => this.handlers.onAddBackground(item, centered)}
				className="rde-editor-items-item"
				style={{ justifyContent: this.state.collapse ? 'center' : null }}
			>
				<span className="rde-editor-items-item-icon">
	...
	render() {
		...
		const { collapse, textSearch, filteredAvatars, activeKey, svgModalVisible, svgOption } = this.state;
				...
				<Flex flex="1" flexDirection="column" style={{ height: '100%' }}>
					...
					<Scrollbar>
						<Flex flex="1" style={{ overflowY: 'hidden' }}>
							{(textSearch.length && this.renderItems(filteredAvatars)) ||
								(collapse ? (
									<Flex
										flexWrap="wrap"
